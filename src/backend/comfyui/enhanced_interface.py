"""
MaiVid Studio - Enhanced ComfyUI Interface

This module extends the ComfyUI interface with improved workflow handling
and more robust node updating capabilities.
"""
...
class EnhancedComfyUIInterface:
    """Enhanced interface for communicating with ComfyUI with improved workflow handling."""
    ...
    def __init__(self, comfyui_url: str = DEFAULT_COMFYUI_URL):
        ...
        logger.info(f"Initialized Enhanced ComfyUI interface with client ID {self.client_id}")
    ...
    def validate_workflow(self, workflow: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        """
        Validate a workflow to ensure all nodes and links are properly defined.
        ...
        try:
            ...
            required_keys = ['nodes', 'links', 'version']
            ...
            for link in workflow['links']:
                if not isinstance(link, list) or len(link) < 6:
                    return False, f"Invalid link format: {link}"
                
                # Format: [link_id, origin_node_id, origin_slot, target_node_id, target_slot, type]
                link_id, origin_node_id, origin_slot, target_node_id, target_slot, link_type = link[:6]
                
                # Check if origin and target nodes exist
                if origin_node_id not in node_ids:
                    return False, f"Link {link_id} references non-existent origin node: {origin_node_id}"
                
                if target_node_id not in node_ids:
                    return False, f"Link {link_id} references non-existent target node: {target_node_id}"
    ...
    def update_workflow(self, workflow: Dict[str, Any], updates: Dict[str, Any]) -> Dict[str, Any]:
        """
        Update parameters in a workflow with enhanced node handling.
        ...
        """
        logger.info("Using enhanced workflow updater")
        updated_workflow = copy.deepcopy(workflow)
        
        # Collect node types for better debugging
        ...
        for key, value in updates.items():
            try:
                ...
                # Convert node_id to string or int as needed
                try:
                ...
                for node in updated_workflow.get("nodes", []):
                    # Try matching with both string and int versions of node_id
                    if str(node.get("id")) == node_id_str or (node_id_int is not None and node.get("id") == node_id_int):
                        node_found = True
                        updated = False
                        node_type = node.get("type", "Unknown")
                        
                        # 1. Handle CLIPTextEncode nodes (text prompts)
                        if node_type == "CLIPTextEncode" and param == "text":
                            if "widgets_values" in node and len(node["widgets_values"]) > 0:
                                node["widgets_values"][0] = value
                                logger.info(f"Updated {node_type} node {node_id} text prompt: {value}")
                                updated = True
                        
                        # 2. Handle EmptyLatentImage nodes (width, height, batch_size)
                        elif node_type == "EmptyLatentImage" and param in ["width", "height", "batch_size"]:
                            if "widgets_values" in node:
                                param_index = {"width": 0, "height": 1, "batch_size": 2}.get(param)
                                if param_index is not None and len(node["widgets_values"]) > param_index:
                                    node["widgets_values"][param_index] = value
                                    logger.info(f"Updated {node_type} node {node_id} {param}: {value}")
                                    updated = True
                        
                        # 3. Handle SaveImage nodes
                        elif node_type == "SaveImage":
                            if "widgets_values" in node:
                                if param == "output_dir" and len(node["widgets_values"]) > 0:
                                    node["widgets_values"][0] = value
                                    updated = True
                                    logger.info(f"Updated {node_type} node {node_id} output_dir: {value}")
                                elif param == "filename_prefix" and len(node["widgets_values"]) > 1:
                                    node["widgets_values"][1] = value
                                    updated = True
                                    logger.info(f"Updated {node_type} node {node_id} filename_prefix: {value}")
                        
                        # 4. Handle CheckpointLoaderSimple nodes
                        elif node_type == "CheckpointLoaderSimple" and param == "ckpt_name":
                            if "widgets_values" in node and len(node["widgets_values"]) > 0:
                                node["widgets_values"][0] = value
                                updated = True
                                logger.info(f"Updated {node_type} node {node_id} checkpoint: {value}")
                        
                        # 5. Try to update inputs if not already updated
                        if not updated and "inputs" in node:
                            input_updated = False
                            for input_param in node["inputs"]:
                                if input_param.get("name") == param:
                                    input_param["link"] = value
                                    updated = True
                                    input_updated = True
                                    logger.info(f"Updated {node_type} node {node_id} input parameter {param}: {value}")
                                    break
                            
                            # If no matching input found by name, try by index
                            if not input_updated and node["inputs"] and param.isdigit():
                                param_index = int(param)
                                if 0 <= param_index < len(node["inputs"]):
                                    node["inputs"][param_index]["link"] = value
                                    updated = True
                                    logger.info(f"Updated {node_type} node {node_id} input at index {param_index}: {value}")
                        
                        # 6. Try to update generic widgets_values
                        if not updated and "widgets_values" in node and param.isdigit():
                            param_index = int(param)
                            if 0 <= param_index < len(node["widgets_values"]):
                                node["widgets_values"][param_index] = value
                                updated = True
                                logger.info(f"Updated {node_type} node {node_id} widget at index {param_index}: {value}")
                        ...
                        if not updated:
                            ...
                            if "widgets_values" in node:
                                ...
                                logger.debug(f"Widget values: {node['widgets_values']}")
                        else:
                            updated_nodes.add(node_id_str)
                        
                        # We found and processed the node, no need to check other nodes
                        break
        ...
        valid, error = self.validate_workflow(updated_workflow)
        if not valid:
            logger.error(f"Validation error after updates: {error}")
            logger.warning("Using original workflow to avoid errors")
            return workflow
    ...
    def generate_scene(self, 
        ...
        """
        Generate a scene image based on description using enhanced workflow handling.
        ...
        try:
            ...
            # Validate and load the workflow
            try:
                workflow = self.load_workflow(workflow_file)
                valid, error = self.validate_workflow(workflow)
                if not valid:
                    logger.error(f"Workflow validation failed: {error}")
                    # Fix the workflow if possible
                    from .fix_workflow import fix_workflow_file
                    fix_output = workflow_file.replace('.json', '_fixed.json')
                    if fix_workflow_file(workflow_file, fix_output):
                        logger.info(f"Fixed workflow saved to {fix_output}")
                        workflow = self.load_workflow(fix_output)
                    else:
                        return None, f"Workflow validation failed: {error}"
            except Exception as e:
                return None, f"Failed to load workflow: {str(e)}"
                ...
                if node["type"] == "CLIPTextEncode":
                    ...
                    if "widgets_values" in node and len(node["widgets_values"]) > 0:
                        ...
                        if isinstance(widget_value, str):
                            if not any(neg_word in widget_value.lower() for neg_word in ["negative", "bad", "ugly", "low quality"]):
            ...
            if text_node_id:
                ...
                logger.info(f"Using node {text_node_id} for text prompt")
            else:
                # If no suitable text node found, log a warning
                logger.warning("No suitable text input node found in workflow")
            ...
            if save_node_id:
                ...
                logger.info(f"Using node {save_node_id} for saving images")
            ...
            for node in updated_workflow["nodes"]:
                if node["type"] in ["KSampler", "SamplerCustom"] and "widgets_values" in node and len(node["widgets_values"]) > 0:
                    ...
                    logger.debug(f"Set random seed for {node['type']}: {node['widgets_values'][0]}")
...
# Compatibility layer for the original interface
class ComfyUIInterface(EnhancedComfyUIInterface):
    """Compatibility class that inherits from the enhanced interface."""
    pass
...
if __name__ == "__main__":
    # Initialize the ComfyUI interface
    comfyui = EnhancedComfyUIInterface()
